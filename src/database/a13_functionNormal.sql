...
-- # 중첩 함수 : 함수에서 그 결과를 가지고, 다시 포함하는 함수의 매개변수로 전달해서, 데이터를 처리
-- 1. 함수 적용1 : 짝수 0, 홀수 1 - 이러한 0, 1 결과값을 가지고,
-- 2. 함수 적용2 : decode라는 함수를 통해서 0일때는 홍팀이 처리되게 하고,
--					그외는 모두 청팀으로 처리되는 것을 말한다.
SELECT ENAME ,EMPNO ,
